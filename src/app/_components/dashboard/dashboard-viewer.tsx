// src/app/_components/dashboard/bi-viewer.tsx
"use client";
...
export default function BIViewer({ dashboardUrl }: BIViewerProps) {
    ...
    return (
        <div
            className=" bg-gray-100, dark:bg-gray-900, flex, flex-col, items-center, justify-center, relative"
            onContextMenu={(e) => e.preventDefault()}
            style={{ height: '89vh', overflow: 'hidden' }} 
        >
            <div className="p-0 rounded-none shadow-none w-full h-full relative"> {/* Adjusted padding, rounded and shadow, added h-screen */}
                {isLoading && (
                    <div className="absolute inset-0 flex items-center justify-center bg-gray-100 dark:bg-gray-800 rounded-none z-10"> {/* Adjusted rounded */}
                ...
                <div className="aspect-w-16 aspect-h-9 h-full"> {/* Added h-full to aspect ratio container */}
                    <iframe
                        ref={iframeRef}
                        // Conditionally set src attribute
                        {...(currentIframeUrl ? { src: currentIframeUrl } : {})}
                        title="BI Dashboard"
                        className="w-full h-full border-0 rounded-none" // Adjusted rounded to rounded-none
                        allowFullScreen
